
#include "includes.h"
#include "externals.h"

namespace UdSDK {

	udPBase GetProjectSOrder( ) {
		return new udProjectSOrder;
	} // udPBase GetProjectSOrder

	CProjectSOrder::CProjectSOrder( ) {
		this->get( "id"			)->ini(	( udUInt )	0	);
		this->get( "name"		)->ini( wstring( L"" )	);
	}

	CProjectSOrder::~CProjectSOrder( ) {
	}

	void CProjectSOrder::GetConfig( udBaseConfig& objConfig ) {
		udBase::GetConfig( objConfig );
		// общие настройки
		objConfig.SetSQLTable( "ud_sorder" );
		objConfig.SetPrefix( "sorder_" );
		objConfig.SetSelectBy( "id" );
		objConfig.SetUpdateBy( "id" );
		objConfig.SetDeleteBy( "id" );
		// настройки атрибутов
		objConfig.SetAttrType( "id", udBaseConfig::UDSDK_TYPE_INTKEY );
		// заголовки атрибутов
		objConfig.SetAttrTitle( "id",	"ID"			);
		objConfig.SetAttrTitle( "name",	"Наименование"	);
	} // void GetConfig

} // namespace UdSDK
