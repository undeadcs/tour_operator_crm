...
namespace UdSDK {
	...
	CHCurrency::CHCurrency( ) : udStateHandlerInfo( ) {
		m_szTableName	= "ud_currency";
		m_szUpdIndex	= "id";
		m_szName		= "currency";
		m_fnGetObject	= GetProjectCurrency;
		m_szOrderBy		= "id";
		m_fnGetFormHandler	= GetHCurrencyForm;
	...
	udInt CHCurrency::GetListModel( ) {
		...
		//////////////////////////////////////////////////////////////////////////////////////////
		title = ( modal ? L"Добавить валюту" : L"Новая валюта" );
		url = "#form";
		lnk = new udWndLink( size );
		lnk->get( "title" )->ini( title );
		lnk->get( "url" )->ini( url );
		...
		wndInsLst->AddAttr( "code", L"Код" )->get( "width" )->ini( ( udInt ) 150 );
		//////////////////////////////////////////////////////////////////////////////////////////
		...
		m_wndModeList->AddInput( fltCode, "code", NULL, font, 0 );
	...
	udInt CHCurrency::GetListView( ) {
		...
		box->padL = 3; box->padT = 2; box->padR = 3; box->padB = 2;
		box->border = 1; box->border_color = Color::MakeARGB( 255, 180, 180, 180 );
		info = m_wndModeList->GetInfo( fltCode );
		info->box = box;
		//////////////////////////////////////////////////////////////////////////////////////////
		if ( modal ) {
			x = m;
	...
	udInt CHCurrency::GetFormView( ) {
		...
		box->padL = 3; box->padT = 2; box->padR = 3; box->padB = 2;
		box->border = 1; box->border_color = Color::MakeARGB( 255, 180, 180, 180 );
		info = m_wndForm->GetInfo( ctrlidCode );
		info->box = box;
		y += h + m;
		//////////////////////////////////////////////////////////////////////////////////////////
		w = 107;
	...
	udInt CHCurrency::ValidateForm( ) {
		...
		wstring code = obj->get( "code" )->toWString( );
		if ( code.empty( ) ) {
			mb::err( hWnd, L"Введите код" );
			return FormHandlerErrorFailed;
		} else if ( code.size( ) > 3 ) {
			mb::err( hWnd, L"Код не более 3-х символов" );
			return FormHandlerErrorFailed;
		}
		return FormHandlerErrorOk;
	} // udInt ValidateForm

	void CHCurrency::ClearFilter( ) {
		udPMorph filter = m_wndModeList->GetObjFilter( );
		filter->get( "name" )->ini( wstring( L"" ) );
		filter->get( "code" )->ini( wstring( L"" ) );
	} // void ClearFilter

	void CHCurrency::Show( udPWndModel wnd ) {
	...
	void CHCurrency::LinkClick( udPWndLink lnk ) {
		if ( ( m_iState == INTSTATE_FORM ) && !m_wndForm->IsModal( ) &&
			...
			if ( lnk == m_lnkPrint ) {
				...
				decl->get( "href" )->ini( wstring( L"currency_out.xsl" ) );
				doc->AddDeclaration( L"xml-stylesheet", decl );
				udDELETE( decl );
				//////////////////////////////////////////////////////////////////////////////////
				obj = GetProjectCurrency( );
				obj->apply( m_wndForm->GetObj( ), true, true );
				node = obj->toXml( doc );
				//////////////////////////////////////////////////////////////////////////////////
				node->SetAttribute( L"object_title", L"Валюта" );
				doc->SetRoot( node );
				doc->toString( xml );
				WriteToFile( folder + L"\\out.xml", xml );
				CopyFile( L".\\xsl\\currency_out.xsl", ( folder + wstring( L"\\currency_out.xsl" ) ).data( ), FALSE );
